...
float wholeseason = 0;      /*- ערך מצטבר של כל העונה -*/ 
float lastbeforesleep = 0;  /*- הערך שנמדד רגע לפני השינה -*/ 
float thefloor = 0;    /*- ערך הרצפה כשהמיכל ריק -*/
...
void loop() {
  ...
  if(WiFi.status() != WL_CONNECTED){
   ...
   if (thefloor == 0 || thefloor < (lastbeforesleep-10)) {
      thefloor = sendvalue;
   }
   float sendrealvalue = thefloor - sendvalue; 
   if (sendvalue != thefloor) lastbeforesleep = sendrealvalue;
  
   int x = ThingSpeak.writeField(myChannelNumber, 1, sendrealvalue, myWriteAPIKey);
   if(x == 200){
